...
{
    public static string EmitPage(PageEmitModel page, HouseStyle style)
    {
        var v = page.Vocabulary;
        // A house style base class not already in the default xmlns needs its own prefix, the
        // same way "controls" and "viewModels" already do below.
        var hasCustomBase = style.PageBaseClassNamespace is not null;
        var root = hasCustomBase ? $"base:{style.PageBaseClass}" : style.PageBaseClass;

        var sb = new StringBuilder();
        sb.AppendLine("<?xml version=\"1.0\" encoding=\"utf-8\" ?>");
        sb.AppendLine("<!-- Generated from " + Attr(page.Document.FolderName) + ", page " + Attr(page.PageName) + ". Do not edit. -->");
        sb.AppendLine($"<{root} xmlns=\"http://schemas.microsoft.com/dotnet/2021/maui\"");
        sb.AppendLine("             xmlns:x=\"http://schemas.microsoft.com/winfx/2009/xaml\"");
        if (hasCustomBase)
            sb.AppendLine($"             xmlns:base=\"clr-namespace:{style.PageBaseClassNamespace}\"");
        sb.AppendLine($"             xmlns:controls=\"clr-namespace:{style.ControlsNamespace}\"");
        sb.AppendLine($"             xmlns:viewModels=\"clr-namespace:{style.ViewModelsNamespace}\"");
        sb.AppendLine($"             x:Class=\"{style.ViewsNamespace}.{page.ClassName}\"");
    ...
    public static string EmitCodeBehind(PageEmitModel page, HouseStyle style)
    {
        var sb = new StringBuilder();
        ...
        sb.AppendLine();
        if (style.PageBaseClassNamespace is not null)
            sb.AppendLine($"using {style.PageBaseClassNamespace};");
        sb.AppendLine($"using {style.ViewModelsNamespace};");
        sb.AppendLine();
        sb.AppendLine($"namespace {style.ViewsNamespace};");
        sb.AppendLine();
        sb.AppendLine($"public partial class {page.ClassName} : {style.PageBaseClass}");
        sb.AppendLine("{");
